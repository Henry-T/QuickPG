...
--- 检查坐标是否在Node区域内 只考虑自身尺寸 (取名nodeContainsPoint的原因是find reference不智能，要和containsPoint区分)
-- 要求Node的Anchor是(0, 0)
-- @ret true/false
function Node:nodeContainsPoint(worldX, worldY)
	local worldPoint = CCPoint(worldX, worldY)
	local localPoint = self:getParent():convertToNodeSpace(worldPoint)
	-- print("local pos: ", localPoint.x, localPoint.y)
	local bound = self:boundingBox() --boundingBox() 是本地坐标包围盒
	-- print("bound: ", bound:getMinX(), bound:getMinY(), bound:getMaxX(), bound:getMaxY())
	return bound:containsPoint(localPoint)
end
...
-- Cell尺寸必须相同
-- @index 索引号 1+
-- @cellSize Cell在卷动方向上的尺寸 (横向是宽，纵向是高)
-- @animated 是否播卷动动画
function CCTableView:setFocusIndex(index, cellSize, animated)
	local direction = self:getDirection()
	if direction == kCCScrollViewDirectionVertical then
		local offsetYMin = self:minContainerOffset().y
		local newOffsetY = math.min(offsetYMin + cellSize*(index-1), 0)
		self:setContentOffset(CCPoint(0, newOffsetY), animated)
	elseif direction == kCCScrollViewDirectionHorizontal then
		local offsetXMin = self:minContainerOffset().x
		local newOffsetX = math.min(offsetXMin + cellSize*(index-1), 0)
