...
function UIListView:addBgIf(params)
    ...
    display.newScale9Sprite(params.bg)
        :size(params.viewRect.size.width, params.viewRect.size.height)
        :pos(params.viewRect.origin.x + params.viewRect.size.width/2,
            params.viewRect.origin.y + params.viewRect.size.height/2)
...
function UIListView:scrollListener(event)
    ...
    nodePoint.x = nodePoint.x - self.viewRect_.origin.x
    nodePoint.y = nodePoint.y - self.viewRect_.origin.y
...
function UIListView:layout_()
    ...
    if UIScrollView.DIRECTION_VERTICAL == self.direction then
        width = self.viewRect_.size.width
        ...
        height = self.viewRect_.size.height
    ...
    -- self:setActualRect({x = self.viewRect_.origin.x,
    --  y = self.viewRect_.origin.y,
    --  width = width,
    --  height = height})
    self:setActualRect(CCRectMake(self.viewRect_.origin.x,
                                  self.viewRect_.origin.y,
                                  width,
                                  height))
    ...
    if UIScrollView.DIRECTION_VERTICAL == self.direction then
        ...
        for i,v in ipairs(self.items_) do
            ...
            v:setPosition(self.viewRect_.origin.x,
                self.viewRect_.origin.y + tempHeight)
        end
    else
        itemW, itemH = 0, 0
        tempWidth = 0
        ...
        for i,v in ipairs(self.items_) do
            ...
            content:setAnchorPoint(0.5, 0.5)
            content:setPosition(itemW/2, itemH/2)
            v:setPosition(self.viewRect_.origin.x + tempWidth, self.viewRect_.origin.y)
            tempWidth = tempWidth + itemW
        end
    end

    self.container:setPosition(0, self.viewRect_.size.height - self.size.height)
end
