...
function Vector3:Equals (other)
    if tolua.type(other) ~= "Vector3" then
        return false;
    end
    local vector = other;
    return self.x.Equals (vector.x) and self.y.Equals (vector.y) and self.z.Equals (vector.z);
end
...
function Vector3:ToString ()
    return string.format("(%01f, %02f, %03f)",
        self.x,
        self.y,
        self.z)
end
...
function Vector3.__mul(a, d)
    return Vector3:new(a.x * d, a.y * d, a.z * d);
end

-- TODO 自己在函数内判断函数类型
-- TODO 确定左参能否非Vector3
function Vector3.__mul(d, a)
    return Vector3:new(a.x * d, a.y * d, a.z * d);
end
